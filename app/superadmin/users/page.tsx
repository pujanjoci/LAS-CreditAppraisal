...
export default function SuperAdminUsersPage() {
  ...
  const handleAddUser = () => {
    // In a real app, this would hit an API. Here we just update local state for UI demonstration.
    const addedUser = {
      ...newUser,
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-display font-bold text-slate-800">Superadmin User Management</h2>
          <p className="text-sm text-slate-500 mt-1">Manage all staff, admin, and superadmin users.</p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
              ...
              <div>
                ...
                <Select value={newUser.role} onChange={(e) => setNewUser({...newUser, role: e.target.value})}>
                  <option value="initiator">Initiator (Staff)</option>
                  <option value="supporter">Supporter (Staff)</option>
                  <option value="reviewer">Reviewer (Staff)</option>
                  <option value="approver">Approver (Staff)</option>
